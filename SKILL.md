...
description: 当业务人员或产品人员用自然语言描述业务需求、页面交互、数据模型、系统设计、接口设计或数据库设计，并需要转成可运行前端、mock 契约、后端开发文档、API 文档或 SQL 交付物时使用；该 skill 用于按 Web SPA、Web SSR/BFF、App placeholder 和 monorepo/workspace 范围路由前端交付，生成可部署 Web 前端工程、前端 mock 假数据对接、按业务模块拆分且可供后续 AI 直接开发的 Java 后端开发文档、REST 风格 HTTP 接口文档和 MySQL SQL 文件；后端只生成开发文档和接口文档，不直接生成 Java 实现代码。
---
...
- Monorepo / workspace scope: when the request creates or changes multiple apps, shared packages, Web + App surfaces, shared DTOs/tools/UI, or a pnpm workspace, load `references/monorepo-workspace.md` in addition to the selected SPA/SSR/App lane references. New monorepo projects default to `apps/*` for runnable applications and `packages/*` for shared packages.
...
1. Classify the request.
   ...
   - For existing or rebuilt projects, produce a migration status matrix before broad changes. Cover UI library, visual parity risk, routing, request wrapper, mock/OpenAPI/Prism, env modes, i18n, DTO placement, feature/shared architecture, monorepo/workspace layout when present, docs/api, docs/backend, SQL, package manager, and quality tooling. Mark each item as `compliant`, `migrated`, `compatible-exception`, or `non-compliant`; do not silently finish with `non-compliant` items.
   ...
   - For a new monorepo, multi-app product, or workspace migration, read `references/monorepo-workspace.md` before choosing package boundaries. Default to `apps/*` for application domains and `packages/*` for shared domains. Shared packages must at least evaluate `packages/types`, `packages/tools`, and `packages/ui`; use pnpm `catalog` for core dependency versions.
...
3. Build or update the frontend.
   ...
   - If the scope is or should become a monorepo, read `references/monorepo-workspace.md` before creating directories, package names, workspace scripts, shared DTOs, shared tools, shared UI, or pnpm catalog entries.
...
- Prism mock contract: single-app project root `mock/openapi.yaml`; in monorepo, the owning app uses `apps/<app-name>/mock/openapi.yaml`, with schemas/requestBodies/responses/parameters under that app's `mock/components/` and examples under that app's `mock/examples/`.
- Mock or API services: feature service directories or the existing service directory; all methods must map to the owning app's `mock/openapi.yaml` operations.
...
- New monorepo projects default to pnpm workspace with `apps/*` and `packages/*`; shared packages default to `packages/types`, `packages/tools`, and `packages/ui` when those domains are needed. Use pnpm `catalog` to control core dependency versions across the workspace.
...
- Use `interface` names with an `I` prefix and semantic PascalCase `type` aliases in TypeScript when creating new TypeScript code. Do not force a `T` prefix; use business names such as `OrderStatus` for enum-like unions or status types.
...
- `references/monorepo-workspace.md`: pnpm workspace, `apps/*` and `packages/*`, shared `types` / `tools` / `ui` packages, client/server/shared runtime entries, dependency boundaries, catalog versions, optional Turbo decision rules, optional e-commerce platform recommendations, and monorepo delivery gates.
...
- `references/quality-tooling.md`: single-app and monorepo ESLint, Prettier, EditorConfig, Commitlint, Husky, lint-staged, inherited root configs, architecture boundaries, and check scripts.
