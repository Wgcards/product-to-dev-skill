...
import type { IMetricItem, TMetricTone } from "../types/workspace";
    ...
    return (
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <article key={index} className="min-h-32 animate-pulse rounded-md border border-border bg-surface p-4">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {metrics.map((metric) => {
        ...
        return (
          ...
          <article key={metric.label} className="min-h-32 rounded-md border border-border bg-surface p-4 transition hover:border-primary/60">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">{metric.label}</p>
              <span className={`inline-flex size-8 items-center justify-center rounded-md ${toneClassName[metric.tone]}`}>
